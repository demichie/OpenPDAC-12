...
//---------------------------------------------------------------
scalar inverseDistanceInterpolationDz(const point& internalPoint, const pointField& boundaryPoints, const scalarField& boundaryDz)
{
    ...
    scalarField distances(boundaryPoints.size());
    scalarField weights(boundaryPoints.size());

    distances = mag(internalPoint - boundaryPoints);

    scalar minValue = gMin(distances);
    ...
    {
        weights = pow((Ldef / distances), a) + pow((alpha * Ldef / distances), b);  
        interpolatedDz = gSum(weights*boundaryDz)/gSum(weights);   
    }
...
//---------------------------------------------------------------------------------------------
    ...
    pointField bottomCentres(patchBottom.size());
    ...
    forAll(patchBottom, facei)
    {
        ...
        const face& f = mesh.faces()[patchBottom.start() + facei];  // Access the face directly from the patch
        

        Info << magFaceAreas[patchBottom.start() + facei] << endl;
        Info << faceCentres[patchBottom.start() + facei] << endl;
        Info << faceNormals[patchBottom.start() + facei] << endl;
        ...
        if (colIndex >= 0 && colIndex <= ncols  && rowIndex >= 0 && rowIndex <= nrows )
        {
            ...
            Info << "zInterp " << zInterp << endl;
            bottomCentres[facei].x() = faceCentres[patchBottom.start() + facei].x();
            bottomCentres[facei].y() = faceCentres[patchBottom.start() + facei].y();
            bottomCentres[facei].z() = faceCentres[patchBottom.start() + facei].z();
    ...
    pointField topCentres(patchTop.size());
    ...
    forAll(patchTop, facei)
    {
        ...
        Info << magFaceAreas[patchTop.start() + facei] << endl;
        Info << faceCentres[patchTop.start() + facei] << endl;
        Info << faceNormals[patchTop.start() + facei] << endl;
        ...
        if (colIndex >= 0 && colIndex <= ncols  && rowIndex >= 0 && rowIndex <= nrows )
        {
            topCentres[facei].x() = faceCentres[patchTop.start() + facei].x();
            topCentres[facei].y() = faceCentres[patchTop.start() + facei].y();
            topCentres[facei].z() = faceCentres[patchTop.start() + facei].z();
            dzTop[facei] = 0.0;
        }
    ...
    pointField concatenatedCentres(bottomCentres.size() + topCentres.size());
    scalarField concatenatedDz(dzBottom.size() + dzTop.size());

    // Copy the first field into the new field
    for (label i = 0; i < bottomCentres.size(); ++i)
    {
        concatenatedCentres[i] = bottomCentres[i];
        concatenatedDz[i] = dzBottom[i];
    }

    // Copy the second field into the new field, starting after the end of the first
    for (label i = 0; i < topCentres.size(); ++i)
    {
        concatenatedCentres[i + bottomCentres.size()] = topCentres[i];
        concatenatedDz[i + dzBottom.size()] = dzTop[i];
    }
      ...
      forAll(pDeform,pointi)
        {
          ...
          interpDz = inverseDistanceInterpolationDz(mesh.points()[pointi], concatenatedCentres, concatenatedDz);
          
          Info << pointi << " " << interpDz << endl; 
