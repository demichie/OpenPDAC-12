...
void Foam::CellZoneInjection<CloudType>::topoChange()
{
    ...
    else
    {
        ...
        // Construct parcel densities
        densities_.setSize(injectorCoordinates_.size());
        forAll(densities_, i)
        {
            densities_[i] = densityDistribution_->sample();
        }
...
)
{
    ...
    parcel.rho() = densities_[parcelI];
    
    parcel.phi() = sfericity_;
